...
from bootleg.symbols.constants import DISAMBIG, REL_INDICES_KEY, REL_EMB_INDICES_KEY, FINAL_LOSS
...
class Bootleg(AttnNetwork):
    ...
    def forward(self, alias_idx_pair_sent, sent_embedding, entity_embedding, batch_prepped, batch_on_the_fly_data):
        ...
        if REL_INDICES_KEY in batch_on_the_fly_data or REL_INDICES_KEY in batch_prepped:
            if REL_INDICES_KEY in batch_on_the_fly_data:
                ...
                kg_bias = batch_prepped[REL_INDICES_KEY].float().to(sent_embedding.tensor.device).reshape(batch_size, self.M * self.K, self.M * self.K)
        else:
        ...
        for stage_index in range(self.num_model_stages):
            ...
            if stage_index < self.num_model_stages-1:
                pred_final = self.predict_layers[DISAMBIG][train_utils.get_stage_head_name(stage_index)](context_matrix)
                pred_final = pred_final.transpose(0,1).squeeze(2).reshape(batch_size, self.M, self.K)
                out[DISAMBIG][train_utils.get_stage_head_name(stage_index)] = pred_final

            query_tensor = context_matrix

        context_matrix = context_matrix.transpose(0,1).reshape(batch_size, self.M, self.K, self.hidden_size)
        context_mat_dict = {"context_matrix_main": context_matrix}
...
class BootlegV1(AttnNetwork):
    ...
    def forward(self, alias_idx_pair_sent, sent_embedding, entity_embedding, batch_prepped, batch_on_the_fly_data):
        ...
        # context_mat_dict is the contextualized entity embeddings, out is the predictions
        context_mat_dict = {"context_matrix_nokg": context_matrix_nokg, "context_matrix_main": context_matrix_main}
        return context_mat_dict, out
