...
parser.add_argument('--out', type=str, help="Save model predictions to this dir.")
parser.add_argument('--bootleg_qid_name', type=str, default='../../data/wiki_entity_data/entity_mappings/qid2title.json', help="Path to the qid2title.json file in bootleg downloads")
parser.add_argument('--outfile', type=str, default='results.json', help="Save raw predictions to this file.")
...
if len(pred_file) > 0:
    ...
    for i in range(len(data)):
        ...
        if predictions[i]  != data[i]["relation"] or 1: #just save it all
            ...
            errors[i]['num_ner'] = len([ner for ner in data[i]['stanford_ner'] if ner != 'O'])
            errors[i]['prop_ner'] = errors[i]['num_ner']/len(errors[i]['stanford_ner'])
            ...
            errors[i]['prop_mentions'] = errors[i]['real_mentions']/len(errors[i]['mentions'])


# Convert the bootleg entity QIDs to the wikidata mentions
...
if len(args.out) > 0:
    ...
    filename_probs = args.out + 'probs.pkl'
