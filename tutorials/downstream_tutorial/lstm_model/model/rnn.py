...
class PositionAwareRNN(nn.Module):
    ...
    def __init__(self, opt, emb_matrix=None, ent_emb_matrix=None):
        ...
        if (
            "ent_emb_dim" in opt
            and opt["ent_emb_dim"] > 0
            and "ent_vocab_size" in opt
            and opt["ent_vocab_size"] > 0
        ):
            self.ent_emb = nn.Embedding(
                opt["ent_vocab_size"], opt["ent_emb_dim"], padding_idx=constant.PAD_ID
            )
        if opt["pos_dim"] > 0:
        ...
        input_size = (
            opt["emb_dim"] + opt["ent_emb_dim"] + opt["pos_dim"] + opt["ner_dim"]
        )
        self.rnn = nn.LSTM(
            input_size,
            opt["hidden_dim"],
    ...
    def init_weights(self):
        ...
        if self.ent_emb is not None:
            if self.ent_emb_matrix is None:
                self.ent_emb.weight.data[1:, :].uniform_(
                    -1.0, 1.0
                )  # keep padding dimension to be 0
            else:
